...
static int (*default_getaddrinfo)(const char *, const char *,
                                  const struct addrinfo *,
                                  struct addrinfo **) = NULL;
static void (*default_freeaddrinfo)(struct addrinfo *) = NULL;

static void heaph_printf(const char *format, ...) {
  va_list args;
...
static void heaph_assert_do(bool flag, const char *expr, int line) {
  if (flag) return;
...
static void spinlock_acq(bool *lock) {
  while (true) {
    int i = 0;
    while (i++ < 1000) {
      if (!__atomic_test_and_set(lock, __ATOMIC_SEQ_CST)) return;
    }
    usleep(10);
  }
}

static void spinlock_rel(bool *lock) {
  heaph_assert(*lock);
  __atomic_clear(lock, __ATOMIC_SEQ_CST);
}

static bool alloc_is_static(const void *ptr) {
  return ptr != NULL && ptr >= (void *)static_buf &&
         ptr <= (void *)(static_buf + static_size);
}

static void alloc_trace_new(void *ptr, size_t size) {
  if (alloc_is_static(ptr)) return;
  ...
  if (is_exit_done) return;
  spinlock_acq(&allocs_lock);
  struct allocation *a = alloc_pool;
  if (a != NULL) {
    alloc_pool = a->next;
  } else {
    if (alloc_batch == NULL || alloc_batch->used == ALLOCATION_BATCH_SIZE) {
      alloc_batch = mmap(NULL, sizeof(*alloc_batch), PROT_READ | PROT_WRITE,
...
static void static_allocator_touch(void) {
  if (static_buf != NULL) return;
  static_size = 16 * 1024;
...
static void *static_malloc(size_t size) {
  ...
  if (content_mode == CONTENT_MODE_TRASH) memset(res, '#', size);
  static_used += size;
  return res;
}
...
static void static_free(void *ptr) {
  ...
  heaph_assert(init_lock);
  static_allocator_touch();
  if (ptr != NULL) heaph_assert(alloc_is_static(ptr));
  (void)ptr;
}

static void *static_calloc(size_t count, size_t size) {
  size *= count;
...
static void heaph_lock(void) {
  if (init_lock_count == 0) spinlock_acq(&init_lock);
  ++init_lock_count;
}

static void heaph_unlock(void) {
  heaph_assert(init_lock_count > 0);
  if (--init_lock_count == 0) spinlock_rel(&init_lock);
}

static bool trace_sym_is_func(const struct symbol *sym, const char *func) {
  if (sym == NULL || sym->name == NULL) return false;
  return strcmp(sym->name, func) == 0;
}

static bool trace_sym_file_starts_with(const struct symbol *sym,
                                       const char *file) {
  if (sym == NULL || sym->file == NULL) return false;
  size_t len = strlen(file);
  return strncmp(sym->file, file, len) == 0;
}

static bool trace_sym_is_internal(const struct symbol *sym) {
  if (trace_sym_is_func(sym, "puts")) return true;
  if (trace_sym_is_func(sym, "pthread_create")) return true;
  if (trace_sym_is_func(sym, "_IO_printf")) return true;
  if (trace_sym_is_func(sym, "_IO_puts")) return true;
  if (trace_sym_is_func(sym, "_IO_vfscanf")) return true;
  if (trace_sym_file_starts_with(sym, "libpthread.so")) return true;
  return false;
}

static bool trace_is_internal(const struct symbol *syms, int count) {
  for (int i = 0; i < count; ++i) {
    if (trace_sym_is_internal(&syms[i])) return true;
  }
  return false;
}

static int trace_resolve(void *const *addrs, int count, struct symbol *syms) {
  int failures = 0;
  Dl_info info;
  for (int i = 0; i < count; ++i) {
...
static void heaph_atexit(void) {
  // On Mac somehow this atexit handler gets executed 2 times. Have to
  // manually filter out all calls except for the first one.
  if (__atomic_test_and_set(&is_exit_done, __ATOMIC_SEQ_CST)) return;
  if (report_mode == REPORT_MODE_QUIET) return;
  spinlock_acq(&allocs_lock);
  ...
  for (; a != NULL; a = a->next) {
    ...
    if (!is_internal) leak_size += a->size;
  }
  spinlock_rel(&allocs_lock);
  ...
  if (count == 0 && report_mode != REPORT_MODE_VERBOSE) return;
...
static void heaph_init(void) {
  ...
  int (*sym_getaddrinfo)(const char *, const char *, const struct addrinfo *,
...
static void heaph_touch(void) {
  if (__atomic_load_n(&is_init_done, __ATOMIC_SEQ_CST)) return;
  if (init_lock_count > 0) return;
...
char *strdup(const char *ptr) {
  ...
  if (res != NULL) alloc_trace_new(res, strlen(res) + 1);
  --depth;
  return res;
}
...
void *calloc(size_t num, size_t size) {
  ...
  if (res != NULL) alloc_trace_new(res, num * size);
  --depth;
  return res;
}
...
void *realloc(void *ptr, size_t size) {
  heaph_touch();
  ++depth;
  if (alloc_is_static(ptr)) ptr = NULL;
...
int getaddrinfo(const char *hostname, const char *servname,
  ...
  if (rc == 0 && *res != NULL) alloc_trace_new(*res, sizeof(**res));
  --depth;
  return rc;
}
